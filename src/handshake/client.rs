...
pub fn generate_request(mut request: Request, config: &Option<WebSocketConfig>) -> Result<(Vec<u8>, String)> {
    let mut req = Vec::new();
    write!(
        req,
        "GET {path} {version:?}\r\n",
...
impl VerifyData {
    pub fn verify_response(&self, response: Response, _config: &Option<WebSocketConfig>) -> Result<(Response, Option<Extensions>)> {
